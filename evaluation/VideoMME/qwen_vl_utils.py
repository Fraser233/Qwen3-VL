...
def _read_video_torchvision(
    ...
    if isinstance(info, dict):
        for k in ("video_fps", "fps", "avg_fps"):
            v = info.get(k, None)
            try:
    ...
    if (video_fps is None or video_fps <= 0) and total_frames > 0:
        video_start = ele.get("video_start", None)
        video_end = ele.get("video_end", None)
        try:
            if video_start is not None and video_end is not None and float(video_end) > float(video_start):
                video_fps = float(total_frames) / max(float(video_end) - float(video_start), 1e-6)
        except Exception:
            video_fps = None

    if video_fps is None or video_fps <= 0:
        video_fps = float(ele.get("fps", FPS))
        logger.warning(f"torchvision metadata has no usable FPS for {video_path}; fallback to {video_fps}.")
